...
name: Traditional Pulumi Preview and Deploy
on: [push]
jobs:
  pulumi:
...
name: Container-Based Pulumi Preview and Deploy
on: [push]
jobs:
  pulumi:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v2
      - name: Pulumi Preview
        uses: docker://internal/pulumiOps:v1.0.0
        with:
          args: preview --stack dev
      - name: Pulumi Up
        uses: docker://internal/pulumiOps:v1.0.0
        with:
          args: up --stack dev --yes
```
